...
class MPassbookActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        filter.setOnClickListener {
            clickDatePicker(it, et_date1 , et_date2  ,filter,ID!!,pass!!,acc_no!!, recyclerView)
        }
        recyclerView.layoutManager = LinearLayoutManager(this)
        items = fetchData(ID!!,pass!!,acc_no!!)
        val adapter: MPassbookAdapter = MPassbookAdapter(items)
        recyclerView.adapter = adapter
    }

    private fun reloader(ID:String, pass:String,acc_no:String, recyclerView : RecyclerView){
        items = fetch2Data(ID!!,pass!!,acc_no!!)
        val adapter: MPassbookAdapter = MPassbookAdapter(items)
        recyclerView.adapter = adapter
    }
    ...
    fun GetTextSQL(id : String, pass : String,acc_no:String) : ArrayList<TransactionData> {
        ...
        try{
            ...
            if(connect!=null) {
                val query : String = "Select TNo,amount,DOT,TransactionType,SenderAccNo,ReceiverAccno from manager_transactions_view where SenderAccNo = $acc_no or ReceiverAccNo = $acc_no ORDER BY DOT DESC"
    ...
    fun GetFilterTextSQL(id : String, pass : String,acc_no:String, datefrom:String, dateto:String) : ArrayList<TransactionData> {
        ...
        try{
            ...
            val connect : Connection = connectionhelper.connectionclass(id,pass)
            if(connect!=null) {
                val query : String = "Select TNo,amount,DOT,TransactionType,SenderAccNo,ReceiverAccno from manager_transactions_view where (SenderAccNo = $acc_no or ReceiverAccNo = $acc_no) and (DOT >= '$datefrom' and DOT <= '$dateto') ORDER BY DOT DESC"
                val st : Statement = connect.createStatement()
                val rs : ResultSet = st.executeQuery(query)
                if (!rs.next()) {
                    println("ResultSet in empty in Java")
